...
export function useInfiniteScroll ({ callback, wrapperRef, triggerRef }: UseInfiniteScrollOptions) {
	...
	useEffect(() => {
		...
		if (callback) {
			const options = { // дэфолтные настройки АПИ | https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API
				root: wrapperElement,
				rootMargin: '20px', // Отступы вокруг root (обязательно при наличии root)
				threshold: 1.0,
			};

			observer.current = new IntersectionObserver(([entry]) => { // массив отслеживаемых объектов
				if (entry.isIntersecting) { // объект появился в области видимости
					callback();
				}
			}, options);
		...
		return () => { // демонтирование наблюдения / отписываемся
			if (observer.current && triggerElement) { // triggerElement - избавляет от null ошибки
