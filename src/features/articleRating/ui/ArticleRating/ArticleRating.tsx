...
const ArticleRating = memo((props: ArticleRatingProps) => {
	...
	const { data, isLoading } = useGetArticleRating({ // сгенерированный РТК хук
		articleId,
		userId: userData?.id ?? '',
	});
	const [rateArticleMutation] = useRateArticle(); // сгенерированный РТК хук (мутации)

	const handleRateArticle = useCallback((starsCount: number, feedback?: string) => { // отправка на сервер
		try {
	...
	const onAccept = useCallback((starsCount: number, feedback?: string) => { // рейтинг + фидбэк
		handleRateArticle(starsCount, feedback);
	}, [handleRateArticle]);

	const onCancel = useCallback((starsCount: number) => { // только рейтинг
